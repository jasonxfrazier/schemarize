...
def create_jsonl_file(
    tmp_path: Path, lines: list[Dict[str, Any]], suffix: str = ""
) -> Path:
...
def create_json_array_file(
    tmp_path: Path, arr: list[Dict[str, Any]], suffix: str = ""
) -> Path:
...
def create_csv_file(
    tmp_path: Path, rows: list[Dict[str, Any]], headers: list[str], suffix: str = ""
) -> Path:
...
def test_read_dataframe_pandas():
    ...
    assert normalize_nans(list(read_dataframe(df))) == normalize_nans(
...
def test_read_dataframe_pyarrow():
    ...
    assert normalize_nans(list(read_dataframe(table))) == normalize_nans(
        df.to_dict(orient="records")
    )
